...
for i in DataLB.getDataKeys():
  for j in (True, False):
    ...
    Data.saveHistogram(h, "RESULTS/laser_blue/1D_{0}{1}_EE.pdf".format(i.replace("/", "."), ("", "_RMS")[j]))
    del h
    h = DataLB.get2DHistogram(i, j, plottype="endcap")
    Data.saveHistogram(h, "RESULTS/laser_blue/2D_{0}{1}_EE.pdf".format(i.replace("/","."), ("", "_RMS")[j]), plottype = "endcap")
